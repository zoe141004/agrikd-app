...
def provision(token_data, force=False):
    ...
    if existing:
        ...
        patch_body = {
            ...
            "status": "unassigned",
            "user_id": None,
            "device_token": None,  # Will be regenerated by DEFAULT
        }
        # We need to use the existing token to authenticate the PATCH,
        # but we also want a new token. Use admin-level anon approach:
        # Actually, "Device self-register" policy won't work for UPDATE.
        # For re-register, we rely on the anon key + existing token header.
