...
export default function DashboardPage() {
  ...
  const loadDashboard = async (showSpinner, signal) => {
    ...
    const ab = (q) => signal ? q.abortSignal(signal) : q
    ...
    const [
      ...
      ab(supabase.rpc('get_dashboard_stats', { p_leaf_type: rpcFilter })),
      ab(supabase.rpc('get_disease_distribution', { p_leaf_type: rpcFilter })),
      ab(supabase.rpc('get_leaf_type_options')),
      ab(supabase.from('model_registry').select('*', { count: 'exact', head: true })),
      ab(addFilter(supabase.from('predictions').select('created_at').gte('created_at', new Date(Date.now() - 30 * 86400000).toISOString()).order('created_at', { ascending: true }).limit(10000))),
      ab(addFilter(supabase.from('predictions').select('id, leaf_type, predicted_class_name, created_at').order('created_at', { ascending: false }).limit(8))),
      ab(supabase.from('devices').select('*', { count: 'exact', head: true }).in('status', ['online', 'assigned', 'offline'])),
    ])
