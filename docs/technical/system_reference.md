...
│   │   │   └── settings/              # Settings, Evaluation screens
│   │   └── providers/                 # Riverpod providers
│   ├── assets/models/                 # Bundled TFLite models (~0.96 MB each)
│   ├── test/                          # 250 tests across 20 test files
...
│   ├── migrations/                    # 27 SQL migration files (001-027)
│   │   ├── 001_tables.sql
...
│   │   └── 027_normalize_accuracy.sql     # Normalize accuracy from % to [0,1] fraction
...
**Prediction deduplication (migration 025):** Flutter and Jetson both use auto-increment
`local_id` starting from 1, so a single unique index on `(user_id, local_id)` caused
collisions. Migration 025 replaces it with two partial unique indexes:
- `idx_predictions_mobile_dedup`: `(user_id, local_id) WHERE device_id IS NULL` — Flutter rows
- `idx_predictions_device_dedup`: `(user_id, device_id, local_id) WHERE device_id IS NOT NULL` — Jetson rows

Flutter upserts with `onConflict: 'user_id,local_id'`; PostgREST infers the mobile partial index
because Flutter rows always have `device_id IS NULL`. The `device_push_predictions` RPC
sets `source='jetson'` and routes to the device partial index.
...
| Settings | SettingsScreen, EvaluationScreen |
...
4. **DVC**: Google Cloud Storage remote in `.dvc/config` (`gs://agrikd-dvc-data/data`, already configured)
5. **Migrations**: Run 001-027 in order in Supabase SQL Editor
