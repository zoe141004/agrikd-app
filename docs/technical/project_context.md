...
- **Wake-Capture-Sleep**: In periodic mode, camera opens/captures/releases per cycle to reduce heat and sensor wear. `capture_single()` applies a warmup phase (`warmup_delay=3.0s`, `warmup_frames=5`) to allow USB/CSI camera auto-exposure to stabilize before capturing the actual frame in headless periodic mode.
...
- **Device Shadow**: Admin sets `desired_config` (mode, interval, leaf type, model versions) via the dashboard. The Jetson polls for config changes, applies them, and reports `reported_config` as acknowledgement. `config_version` auto-increments on each change.
- **Config Flow (Single-Writer)**: The Admin Dashboard saves `desired_config.model_versions` to the Supabase `devices` table. On-device, `SyncEngine` is the **single writer** for both `device_state.json` and `config.json`; `main.py` reads config exclusively via `sync.get_active_config()` (thread-safe, no network call). On startup, `SyncEngine` pre-checks `desired_config.model_versions` against loaded engines and triggers builds immediately if mismatched.
- **Model Version Management**: Admin assigns specific model versions per leaf_type to each device via the dashboard. The sync engine detects version changes and automatically: downloads cached TensorRT engine (if available for the same hardware tag) or downloads ONNX + builds engine with `trtexec --fp16`. The engine builder discovers `trtexec` by searching: `PATH` → `/usr/src/tensorrt/bin/trtexec` → `/usr/local/bin/trtexec` → `~/trtexec`. Engine files use versioned naming: `{leaf_type}_student_v{version}.engine` (e.g., `tomato_student_v1.1.0.engine`). Engines are hot-swapped in `InferenceWorkerPool` without service restart. Old engine files are deleted after successful hot-swap. Build status (`building`/`ready`/`error`) and applied versions are reported in `reported_config`.
- **Model Version Logging**: Model versions are logged at startup, on hot-swap, and with each prediction using the format `[model=leaf_type vX.Y.Z]`.
...
- **Health endpoint**: `/health` returns `model_versions` dict (per-leaf-type version strings) when the request is authenticated.
